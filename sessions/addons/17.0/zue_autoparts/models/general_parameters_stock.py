...
class stock_class(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            result.append((record.id, "{} | Sistema: {} | División: {}".format(record.name, record.segments_id.name, record.subsegments_id.name)))
        return result
...
class zue_market_research(models.Model):
    ...
    def name_get(self):
        result = []
        for record in self:
            result.append((record.id, "Proveedor: {} | Codigo mercado: {}".format(record.z_supplier_name, record.z_market_code)))
        return result
